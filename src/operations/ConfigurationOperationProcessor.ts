import { Client, IOperation } from '@c8y/client';
import { exec } from 'child_process';
import { MqttClient } from 'mqtt';
import { IConfiguration } from '../models/configuration';
import { OperationProcessor, OperationStatusMessageIds } from '../models/OperationProcessor';

export class ConfigurationOperationProcessor extends OperationProcessor {
  operationType = 'c8y_Configuration';

  async initialize(mqttClient: MqttClient, restClient: Client): Promise<void> {
    this.sendCurrentConfigToCloud(mqttClient);
    return;
  }

  async processOperation(operation: IOperation, mqttClient: MqttClient, restClient: Client): Promise<boolean> {
    if (operation[this.operationType] && operation.externalSource.externalId === this.agentConfig.getClientId()) {
      this.updateOperationStatus(mqttClient, OperationStatusMessageIds.EXECUTING);
      try {
        const { config } = operation[this.operationType];
        let configuration: Partial<IConfiguration>;
        try {
          configuration = JSON.parse(config);
        } catch (e) {
          throw Error('Failed to parse configuration as json object.');
        }

        this.agentConfig.replaceConfiguration(configuration);
        this.sendCurrentConfigToCloud(mqttClient);
        this.updateOperationStatus(mqttClient, OperationStatusMessageIds.SUCCESS);
      } catch (e) {
        this.logger.error(e);
        const escapedString = this.escapeString(e.message || JSON.stringify(e));
        this.updateOperationStatus(mqttClient, OperationStatusMessageIds.FAILED, escapedString);
      }
      return true;
    }
    return false;
  }

  protected performCommand(cmd: string): Promise<string> {
    this.logger.info(`Executing command: ${cmd}`);
    return new Promise<string>((resolve, reject) => {
      const child = exec(cmd, { timeout: 60000 }, (err, stdout, stderr) => {
        if (child.exitCode !== 0) {
          this.logger.info(`exitcode: ${child.exitCode}`);
          return reject(stderr || err?.message);
        } else {
          return resolve(stdout);
        }
      });
    });
  }

  protected sendCurrentConfigToCloud(mqttClient: MqttClient): void {
    const config = this.agentConfig.getWholeConfiguration();

    mqttClient.publish('s/us', `113,${this.escapeString(JSON.stringify(config, undefined, 2))}`);
  }
}
